...
namespace Ultima
{
    ...
    {
        private static FileIndex m_FileIndex = new("Anim.idx", "Anim.mul", 6);
        private static FileIndex m_FileIndex2 = new("Anim2.idx", "Anim2.mul", -1);
        private static FileIndex m_FileIndex3 = new("Anim3.idx", "Anim3.mul", -1);
        private static FileIndex m_FileIndex4 = new("Anim4.idx", "Anim4.mul", -1);
        private static FileIndex m_FileIndex5 = new("Anim5.idx", "Anim5.mul", -1);
        ...
        public static void ExportToVD(int filetype, int body, string file)
        {
            ...
            int index;
            GetFileIndex(body, filetype, 0, 0, out fileIndex, out index);
            using FileStream fs = new(file, FileMode.Create, FileAccess.Write, FileShare.Write);
    ...
    public sealed class AnimIdx
    {
        ...
        public unsafe Bitmap[] GetFrames()
        {
            ...
            for (int i = 0; i < bits.Length; ++i)
            {
                ...
                if (height == 0 || width == 0)
                    continue;
                Bitmap bmp = new(width, height, PixelFormat.Format16bppArgb1555);
        ...
        {
            using MemoryStream imageStreamSource = new();
            System.Drawing.ImageConverter ic = new();
            byte[] btImage = (byte[])ic.ConvertTo(bit, typeof(byte[]));
            imageStreamSource.Write(btImage, 0, btImage.Length);
            GifBitmapDecoder decoder = new(imageStreamSource, BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.Default);
        ...
        public unsafe void ExportPalette(string filename, int type)
        {
            switch (type)
            {
                ...
                case 1:
                    {
                        Bitmap bmp = new(0x100, 20, PixelFormat.Format16bppArgb1555);
